...
for epoch = 1:params.max_epoch
    ...
    for iter = 1:num_samples
        ...
        % Update weights
        condition =  current_label*log2(sigmoid(dot(theta, current_sample))) + (1-current_label)*log2(1-sigmoid(dot(theta, current_sample)))
        if  (condition < 0)
    ...
    error(epoch) = (error(epoch)  / num_samples) + params.lambda*sum(theta.^2);
